...
const StableHerb: React.FC<HerbProps> = ({ index, obstacles }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Enhanced damping with threshold for complete stop
      const dampingX = 0.92
      const dampingY = 0.96
      herb.vx *= dampingX
      herb.vy *= dampingY
      herb.rotationSpeed *= 0.99
      
      // Complete stop for tiny movements
      if (Math.abs(herb.vx) < 0.1) herb.vx = 0
      if (Math.abs(herb.vy) < 0.1) herb.vy = 0
      if (Math.abs(herb.rotationSpeed) < 0.05) herb.rotationSpeed = 0
      ...
      if (distance < 100 && distance > 0) {
        ...
        herb.rotationSpeed += (Math.random() - 0.5) * force * 0.5
      }
      ...
      if (herb.y > window.innerHeight + 100) {
        ...
        herb.rotationSpeed = (Math.random() - 0.5) * 2
      }
      ...
      for (let i = 0; i < herbRegistry.length; i++) {
        ...
        if (distance < herbRadius * 2 && distance > 0) {
          ...
          if (dvDotN > 0) {
            // Apply impulse with reduced restitution
            const restitution = 0.4
            const impulse = 2 * dvDotN / 2
      ...
      for (const obstacle of obstacles) {
        if (obstacle.type === 'circle' && obstacle.radius) {
          ...
          if (distance < obstacle.radius + 50) {
            ...
            if (dotProduct < 0) {
              herb.vx -= 2 * dotProduct * normalX * 0.5
              herb.vy -= 2 * dotProduct * normalY * 0.5
              herb.rotationSpeed = (Math.random() - 0.5) * 3
            }
          }
          ...
          if (distance < herbRadius) {
            ...
            if (minDist === leftDist) {
              herb.vx = Math.max(Math.abs(herb.vx) * 0.5, 0.3)
              finalX = obstacle.x - herbRadius
            } else if (minDist === rightDist) {
              herb.vx = Math.min(-Math.abs(herb.vx) * 0.5, -0.3)
              finalX = obstacle.x + obstacle.width + herbRadius
            } else if (minDist === topDist) {
              herb.vy = -Math.abs(herb.vy) * 0.2 - 0.5
              finalY = obstacle.y - herbRadius
            } else {
              herb.vy = Math.min(-Math.abs(herb.vy) * 0.5, -0.3)
      ...
      // Ensure herb is visible and properly positioned
      herb.visible = true
      herb.opacity = 0.7
...
const StableHerbs: React.FC<{ obstacles: any[] }> = ({ obstacles }) => {
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-30">
      {[...Array(herbCount)].map((_, i) => (
        <StableHerb key={`herb-${i}`} index={i} obstacles={obstacles} />
      ))}
    </div>
  )
}

export default StableHerbs